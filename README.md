# 🌐 Digital Literacy Portfolio
### CSE0001 – Digital Literacy | VIT Bhopal University

---

## 👤 Student Details

| Field | Details |
|---|---|
| **Name** | *Farah Raza* |
| **Registration No.** | *25BHI10027* |
| **Branch** | B.Tech – CSE Health Informatics |
| **Year / Semester** | First Year – Semester (2) |
| **Course Code** | CSE0001 |
| **Course Title** | Digital Literacy |



---

## 📌 Project Overview

This repository is my **Digital Literacy Portfolio**, submitted as part of the CSE0001 course at VIT Bhopal University. In this project, I take on the role of a **Student Digital Ambassador** — helping peers navigate the digital world responsibly and professionally.

The portfolio is organized across **five tasks**, each mapping to one module of the course:

| Task | Module | Topic | Marks |
|---|---|---|---|
| Task 1 | Module 1 | Digital Literacy Infographic | 20 |
| Task 2 | Module 2 | Student Digital Portfolio | 20 |
| Task 3 | Module 3 | Coding & Collaboration Platforms | 20 |
| Task 4 | Module 4 | Professional Email & Etiquette Guide | 20 |
| Task 5 | Module 5 | Cybercrime Awareness Case Study | 20 |
| **Bonus** | — | Repository structure & README quality | +5 |
| **Total** | | | **100** |

---

## 📁 Repository Structure

```
digital-literacy-project/
│
├── README.md                              ← You are here
│
│
├── task-1-presentation/
│   ├── digital-literacy-infographic.svg   ← Exported infographic (Canva)
│   └── Project Report Task 1.pdf          ← Brief description
│
├── task-2-portfolio/
│   ├──README.md                           ←Readme file of Task
│   ├── github-profile.png                 ← Screenshot of GitHub profile
│   ├── linkedin-profile.png               ← Screenshot of LinkedIn profile
│   ├── Research Gate-profile.png          ← Screenshot of Kaggle profile
│   └── Project Report Task 2.md           ← Brief description
│
├── task-3-platforms/
│   ├──README.md                           ←Readme file of Task
│   ├── Completed Challenges.png           ← Completed challenge screenshot
│   ├── badge.png                          ← Badge / score screenshot
│   ├── Google Form.jpg                    ← Google Form screenshot
│   └── Response Sheet.jpg                 ← Linked Google Sheet screenshot
│   └── Project Report Task 3.md           ← Brief description
│
├── task-4-email-etiquette/
│   ├──README.md                           ←Readme file of Task
│   ├── Email 1.md                         ←professional email drafts 1
│   ├── Email 2.md                         ←professional email drafts 2
│   └── Social Media Checklist.md          ← Social media Do's and Don'ts
│   └── Project Report Task 4.md           ← Brief description
│
└── task-5-cybercrime/
    ├──README.md                           ←Readme file of Task
    ├── casestudy.md                       ← UPI fraud case study
    └── prevention-checklist.md            ← Stay Safe Online checklist
    └── Project Report Task 5.md           ← Brief description

```
___

## 📝 Task Summaries

### ✅ Task 1 – Digital Literacy Awareness Infographic
> *Module 1 | 20 marks*

Created a one-page infographic using **Canva** covering three core digital literacy topics: what digital literacy is, safe internet practices, and professional online presence. The visual is designed for first-year students with no prior digital training.

📂 Files: [`task-1-presentation/`](./task-1-presentation/)

---

### ✅ Task 2 – Student Digital Portfolio
> *Module 2 | 20 marks*

Set up professional profiles on three platforms to build a foundational digital presence:

| Platform | Purpose | Profile Link |
|---|---|---|
| **GitHub** | Code hosting, portfolio, open source | *(your GitHub profile URL)* |
| **LinkedIn** | Professional networking, internships | *(your LinkedIn URL)* |
| **Kaggle** | Data science learning and competitions | *(your Kaggle URL)* |

GitHub profile README includes name, branch, year, and learning goals.

📂 Files: [`task-2-portfolio/`](./task-2-portfolio/)

---

### ✅ Task 3 – Coding & Collaboration Platforms
> *Module 3 | 20 marks*

**Part A – Coding Practice (HackerRank):**
Completed the beginner-level *"Solve Me First"* challenge in Python 3. This introduced the HackerRank environment, test case validation, and submission workflow.

**Part B – Google Workspace (Google Forms):**
Created a **Digital Literacy Awareness Quiz** with 5 questions for batchmates, including multiple choice and short answer questions. Responses are collected automatically in a linked Google Sheet.

🔗 **Digital Literacy Quiz:** *(paste your Google Form link here)*

📂 Files: [`task-3-platforms/`](./task-3-platforms/)

---

### ✅ Task 4 – Professional Email & Etiquette Guide
> *Module 4 | 20 marks*

**Part A – Professional Emails:**
- **Email 1:** Student → Professor requesting an assignment deadline extension
- **Email 2:** Student → Internship Coordinator expressing interest in a summer internship

Both emails follow professional standards: clear subject line, formal salutation, structured body, and proper sign-off.

**Part B – Social Media Checklist:**
A Do's and Don'ts checklist with 7 Do's and 7 Don'ts for responsible social media use as a college student.

📂 Files: [`task-4-email-etiquette/`](./task-4-email-etiquette/)

---

### ✅ Task 5 – Cybercrime Awareness Case Study & Prevention Guide
> *Module 5 | 20 marks*

**Crime Type: UPI / Online Payment Fraud**

**Part A – Case Study:**
A realistic scenario based on the widely documented OLX Army Officer scam pattern, where a student selling a laptop is tricked into entering his UPI PIN in response to a fraudulent collect request — debiting his account instead of crediting it.

**Part B – Prevention Checklist:**
*Stay Safe Online* checklist with 12 actionable tips for Indian college students, 4 UPI/financial safety tips, a step-by-step victim response guide, and official reporting channels.

> 🚨 **Report cybercrime at [cybercrime.gov.in](https://cybercrime.gov.in) or call 1930**

📂 Files: [`task-5-cybercrime/`](./task-5-cybercrime/)

---

## 🔗 Quick Links

| Resource | Link |
|---|---|
| 🧠 Digital Literacy Quiz (Google Form) | *(https://docs.google.com/forms/d/e/1FAIpQLSdddD4LODuHYCA37qzrZc61zhbb11_z7JHbKYDh8xJuugS4DQ/viewform)* |
| 💼 LinkedIn Profile | *(https://www.linkedin.com/in/farah-raza-56879337a?utm_source=share_via&utm_content=profile&utm_medium=member_android)* |
| 🐙 GitHub Profile | *(https://github.com/Farah-Raza)* |
| 📊 Research Gate | *(https://www.researchgate.net/profile/Farah-Raza-2?ev=hdr_xprf)* |
| 🚨 Cybercrime Reporting Portal | [cybercrime.gov.in](https://cybercrime.gov.in) |
| 📞 Cyber Helpline | 1930 |


---

## 🛠️ Tools & Platforms Used

| Tool / Platform | Used For |
|---|---|
| [Canva](https://canva.com) | Task 1 – Infographic design |
| [GitHub](https://github.com) | Task 2 – Portfolio + repository hosting |
| [LinkedIn](https://linkedin.com) | Task 2 – Professional networking profile |
| [Research Gate](https://www.researchgate.net/) | Task 2 – Data science profile |
| [HackerRank](https://hackerrank.com) | Task 3 – Coding practice |
| [Google Forms](https://forms.google.com) | Task 3 – Digital literacy quiz |
| Google Sheets | Task 3 – Automated form response collection |
| Markdown (`.md`) | All written deliverables |

---

## 📚 References

1. NPCI – UPI Safety Guidelines: https://www.npci.org.in
2. National Cyber Crime Reporting Portal: https://cybercrime.gov.in
3. Reserve Bank of India – UPI Fraud Advisories: https://www.rbi.org.in
4. VIT Bhopal University – CSE0001 Digital Literacy Project Brief (2025–26)
5. Google Workspace Learning Centre: https://workspace.google.com/learning-center
6. HackerRank Help Centre: https://www.hackerrank.com/help-center

---

*This repository was created as part of the CSE0001 Digital Literacy course at VIT Bhopal University.*  
*All screenshots, written content, and reflections are original work.*  
